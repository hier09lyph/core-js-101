...
function doRectanglesOverlap(rect1, rect2) {
  ...
  if (rect1.top > rect2Bottom || rect1Bottom < rect2.top) {
    return false;
  }
...
function isInsideCircle(circle, point) {
  const dis = ((point.x - circle.center.x) ** 2 + (point.y - circle.center.y) ** 2) ** (1 / 2);
...
function isCreditCardNumber(ccn) {
  const reversedCcn = ccn.toString().split('').reverse().join('');
  let sum = 0;

  for (let i = 0; i < reversedCcn.length; i += 1) {
    let digit = Number(reversedCcn[i]);
...
function getDigitalRoot(num) {
  let answerNum = num;
  while (answerNum > 9) {
    answerNum = answerNum.toString().split('').reduce((acc, val) => acc + +val, 0);
  }
  return answerNum;
}
...
function evaluateTicTacToePosition(position) {
  ...
  for (const combination of winningCombinations) {
    if (combination.every((element) => element === 'X')) {
      return 'X';
    }
